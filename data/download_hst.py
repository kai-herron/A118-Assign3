...
if __name__ == '__main__':
    ...
    cmd_cat = 'wget https://archive.stsci.edu/hlsps/fhufd/hlsp_fhufd_hst_acs_{d}_multi_v2.0_cat.fits'
    for name in UFD_NAMES:
        for band in BANDS:
            if name not in SINGLEFIELD:
                #os.system(cmd.format(d=name+'01',b=band))
                os.system(cmd_cat.format(d=name))
            else:
                #os.system(cmd.format(d=name,b=band))
                os.system(cmd_cat.format(d=name))
